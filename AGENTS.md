...
## 1. 语言与沟通
- 所有回复必须使用中文。
- 项目问题（与当前仓库代码、配置、流程、规划直接相关）需先记录问题并进入讨论，不得直接改代码。
- 非项目问题（通用问答、闲聊、与仓库无关事项）不进入 `worklog`，可直接处理。
- 提示词/规则文档修改（如 `AGENTS.md`、提示词模板）按非项目问题处理：不记录 `worklog`，可直接修改。
- 我提出使用团队分析、团队审核等的时候自动使用 subagent 创建共享团队工作，首先分析好需要的角色，然乎并行启动。
- 注意，在我没有明确指出使用团队讨论时，不得开启 subagent 进行讨论
- 讨论输出结构固定为：
...
## 10. MzAgentSub 版本号规则
- 每次进入“项目代码修改”任务时，默认先将 `MzAgentSub/src/pyproject.toml` 中 `[workspace.package].version` 的最后一位（patch）`+1`。
...
- 在上述约束条件之下，除非特别说明否则不要创建文档、不要测试、不要编译、不要运行、不需要总结
...
- ui相关任务必须使用ui-ux-pro-max这一skills，相关任务完成后使用playwright库预览页面
- Claude会不断审查你生成的内容
